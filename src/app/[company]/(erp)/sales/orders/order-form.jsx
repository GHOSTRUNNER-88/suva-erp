...
/**
 * Shared create/edit form for Sales Orders — unlike the Sheet-based
 * single-entity forms elsewhere (Warehouses, Parties, Items), a document
 * with line items needs real horizontal room, so this renders as the full
 * content of its own page (new/page.js and [order]/page.js both mount it),
 * mirroring purchase/orders' order-form.jsx.
 *
 * `initialValues` is the "Create Order" convenience pre-fill from a Sales
 * Quotation's detail page (query-string driven, see [quotation]/page.js) —
 * only meaningful when `order` is null (create mode). It is NOT a DB link;
 * once submitted this order has no record of which quotation it came from
 * (legacy has none either — see actions.js's file header).
 */
export default function SalesOrderForm({ companySlug, formData, order, lines, initialValues }) {
  ...
  const [form, setForm] = useState(() =>
    order
      ? {
          ...
          discValue: String(Number(order.discType === "percent" ? order.discPercent : order.discAmount)),
          isVatApplicable: order.isVatApplicable === 1,
          vatPercent: String(Number(order.vatPercent ?? formData.defaultVatPercent)),
          notes: order.notes ?? "",
          status: order.status,
        }
      : {
          orderNumber: formData.suggestedOrderNumber,
          orderDate: today(),
          expectedDate: "",
  ...
  function updateLine(key, field, value) {
    setLineRows((current) =>
      current.map((row) => {
        ...
        // Selecting an item defaults this row's unit + rate from that
        // item's own primary unit / selling price — still freely editable
        // afterward (e.g. picking the secondary unit, or a negotiated rate).
  ...
  function buildPayload() {
    return {
      ...
      // schema's .positive() check (see actions.js's lineSchema/orderSchema).
  ...
  async function startSubmit() {
    ...
    const result = order
      ? await updateSalesOrderAction(companySlug, order.id, payload)
      : await createSalesOrderAction(companySlug, payload);
    setPending(false);
    if (!result.ok) {
    ...
    if (order) {
      ...
      router.push(`/${companySlug}/sales/orders/${result.id}`);
    }
  }
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="rounded-xl border p-4 space-y-4">
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          <div className="space-y-1.5">
            ...
            <Input id="order-number" className="h-11" value={form.orderNumber} onChange={(event) => update("orderNumber", event.target.value)} />
            {fieldErrors.orderNumber?.[0] && <p className="text-xs text-destructive">{t(fieldErrors.orderNumber[0])}</p>}
          </div>
          <DualDateField id="order-date" label={t("orderDate")} value={form.orderDate} onChange={(value) => update("orderDate", value)} required />
        </div>

        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          <DualDateField id="order-expected-date" label={t("expectedDate")} value={form.expectedDate} onChange={(value) => update("expectedDate", value)} />
          <div className="space-y-1.5">
            <Label htmlFor="order-warehouse">{t("warehouse")}</Label>
            <CreatableSelect id="order-warehouse" options={warehouseOptions} value={form.warehouseId} onChange={(value) => update("warehouseId", value)} placeholder={t("none")} />
          </div>
        </div>

        <div className="space-y-1.5">
          <Label htmlFor="order-party">{t("party")}</Label>
          <CreatableSelect id="order-party" options={partyOptions} value={form.partyId} onChange={handlePartyChange} placeholder={t("selectParty")} />
          {fieldErrors.partyId?.[0] && <p className="text-xs text-destructive">{t(fieldErrors.partyId[0])}</p>}
        </div>

        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          <div className="space-y-1.5">
            <Label htmlFor="order-billing-name">{t("billingName")}</Label>
            <Input id="order-billing-name" className="h-11" value={form.billingName} onChange={(event) => update("billingName", event.target.value)} />
          </div>
          <div className="space-y-1.5">
            <Label htmlFor="order-pan">{t("panNumber")}</Label>
            <Input id="order-pan" className="h-11" value={form.panNumber} onChange={(event) => update("panNumber", event.target.value)} />
          </div>
        </div>

        <div className="space-y-1.5">
          <Label htmlFor="order-billing-address">{t("billingAddress")}</Label>
          <Input id="order-billing-address" className="h-11" value={form.billingAddress} onChange={(event) => update("billingAddress", event.target.value)} />
        </div>

        <div className="space-y-1.5">
          <Label>{t("status")}</Label>
          <SegmentedControl wrap options={ORDER_STATUSES.map((status) => ({ value: status, label: t(status) }))} value={form.status} onChange={(value) => update("status", value)} />
        </div>
      </div>
      ...
      <div className="flex justify-end gap-2 border-t pt-4">
        <Button type="button" variant="outline" onClick={() => router.push(`/${companySlug}/sales/orders`)} disabled={pending}>
          {t("cancel")}
        </Button>
