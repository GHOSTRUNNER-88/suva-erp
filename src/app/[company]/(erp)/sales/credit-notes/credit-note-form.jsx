...
/**
 * Create/edit form for a Credit Note — mirrors purchase/debit-notes'
 * debit-note-form.jsx (its purchase-side counterpart). Two differences:
 * a creditNoteType selector (sales_return moves stock, price_protection
 * never does — see actions.js), and no draft/status field since
 * actions.js always creates directly as "completed".
 */
export default function CreditNoteForm({ companySlug, mode, creditNote, initialLines, formData }) {
  ...
  const [form, setForm] = useState(() => {
    if (isEdit && creditNote) {
      return {
        ...
        partyId: String(creditNote.partyId),
        referenceNo: creditNote.referenceNo ?? "",
        billingName: creditNote.billingName ?? "",
        billingAddress: creditNote.billingAddress ?? "",
        bankAccountId: creditNote.bankAccountId ? String(creditNote.bankAccountId) : "",
        warehouseId: creditNote.warehouseId ? String(creditNote.warehouseId) : "",
        discType: creditNote.discType,
        discValue:
          creditNote.discType === "percent" ? decimalToInputValue(creditNote.discPercent) : decimalToInputValue(creditNote.discAmount),
        isVatApplicable: creditNote.isVatApplicable === 1,
        vatPercent: decimalToInputValue(creditNote.vatPercent ?? 0),
        isRefunded: creditNote.isRefunded === 1,
        refundAmount: decimalToInputValue(creditNote.refundAmount),
      ...
      current.map((line) => {
        ...
        if (field === "itemId") {
          ...
          if (item) {
            ...
            next.rate = decimalToInputValue(item.sellingPrice);
          }
        }
        return next;
      })
  ...
  async function startSubmit() {
    ...
    const result = isEdit
      ? await updateCreditNoteAction(companySlug, creditNote.id, payload)
      : await createCreditNoteAction(companySlug, payload);
    setPending(false);
    if (!result.ok) {
    ...
    router.push(`/${companySlug}/sales/credit-notes/${isEdit ? creditNote.id : result.id}`);
    router.refresh();
  }
    ...
    <div className="space-y-5">
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {isEdit && (
            <div className="space-y-1.5">
              <Label>{t("creditNoteNumber")}</Label>
              <Input className="h-11" value={creditNote.creditNoteNumber} disabled />
              <p className="text-xs text-muted-foreground">{t("creditNoteNumberHint")}</p>
            </div>
          )}
          <DualDateField
            id="cn-date"
            ...
            onChange={(value) => update("creditNoteDate", value)}
            required
          />
          <div className="space-y-1.5">
            <Label>{t("creditNoteType")}</Label>
            <SegmentedControl
              className="h-11"
              options={[
                { value: "sales_return", label: t("salesReturn") },
                { value: "price_protection", label: t("priceProtection") },
              ]}
              value={form.creditNoteType}
              onChange={(value) => update("creditNoteType", value)}
            />
          </div>
          <div className="space-y-1.5">
            <Label htmlFor="cn-warehouse">
              {t("warehouse")}
              {form.creditNoteType === "sales_return" ? " *" : ""}
            </Label>
            <CreatableSelect
              id="cn-warehouse"
            ...
            {form.creditNoteType === "sales_return" && (
              <p className="text-xs text-muted-foreground">{t("salesReturnWarehouseHint")}</p>
            )}
          </div>

          <div className="space-y-1.5">
            <Label htmlFor="cn-party">{t("customer")}</Label>
            <CreatableSelect id="cn-party" options={partyOptions} value={form.partyId} onChange={selectParty} placeholder={t("selectCustomer")} />
            {fieldErrors.partyId?.[0] && <p className="text-xs text-destructive">{t(fieldErrors.partyId[0])}</p>}
          </div>
          <div className="space-y-1.5">
            <Label htmlFor="cn-reference">{t("referenceNo")}</Label>
            <Input
              id="cn-reference"
              className="h-11"
          ...
          <div className="space-y-1.5">
            <Label htmlFor="cn-billing-name">{t("billingName")}</Label>
            <Input id="cn-billing-name" className="h-11" value={form.billingName} onChange={(event) => update("billingName", event.target.value)} />
          </div>

          <div className="space-y-1.5 sm:col-span-2">
            <Label htmlFor="cn-billing-address">{t("billingAddress")}</Label>
            <Input
              id="cn-billing-address"
              className="h-11"
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        <div className="space-y-4 lg:col-span-2">
          <div className="space-y-1.5">
            <Label htmlFor="cn-notes">{t("notes")}</Label>
            <textarea
              id="cn-notes"
              rows={3}
          ...
          <div className="space-y-3 rounded-2xl border bg-background p-4">
            <div className="flex items-center gap-2">
              <Checkbox id="cn-is-refunded" checked={form.isRefunded} onCheckedChange={(checked) => update("isRefunded", checked === true)} />
              <Label htmlFor="cn-is-refunded" className="cursor-pointer">
                {t("refundedNow")}
              </Label>
            </div>
            ...
            {form.isRefunded && (
              <div className="grid grid-cols-2 gap-3 pt-1">
                <div className="space-y-1.5">
                  <Label htmlFor="cn-bank-account">{t("bankAccount")}</Label>
                  <CreatableSelect id="cn-bank-account" options={bankAccountOptions} value={form.bankAccountId} onChange={(value) => update("bankAccountId", value)} placeholder={t("selectBankAccount")} />
                  {fieldErrors.bankAccountId?.[0] && <p className="text-xs text-destructive">{t(fieldErrors.bankAccountId[0])}</p>}
                </div>
                <div className="space-y-1.5">
                  <Label htmlFor="cn-refund-amount">{t("refundAmount")}</Label>
                  <Input id="cn-refund-amount" type="number" min="0" step="0.01" className="h-11" value={form.refundAmount} onChange={(event) => update("refundAmount", event.target.value)} />
                </div>
              </div>
            )}
          </div>
