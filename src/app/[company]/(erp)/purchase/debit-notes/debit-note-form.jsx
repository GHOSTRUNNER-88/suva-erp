...
/**
 * Create/edit form for a Debit Note (purchase return) — mirrors
 * bill-form.jsx's shape. Two deliberate differences from a Purchase Bill:
 * debitNoteNumber is never a form field (always server-generated at create,
 * never client-editable — see actions.js's file header comment), and there
 * is no panNumber column on debit_notes (see db/schema/organization.js).
 */
export default function DebitNoteForm({ companySlug, mode, debitNote, initialLines, formData }) {
  ...
  const [form, setForm] = useState(() => {
    if (isEdit && debitNote) {
      return {
        ...
        partyId: String(debitNote.partyId),
        referenceNo: debitNote.referenceNo ?? "",
        supplierName: debitNote.supplierName ?? "",
        supplierAddress: debitNote.supplierAddress ?? "",
        bankAccountId: debitNote.bankAccountId ? String(debitNote.bankAccountId) : "",
        warehouseId: debitNote.warehouseId ? String(debitNote.warehouseId) : "",
        discType: debitNote.discType,
        discValue: debitNote.discType === "percent" ? decimalToInputValue(debitNote.discPercent) : decimalToInputValue(debitNote.discAmount),
        isVatApplicable: debitNote.isVatApplicable === 1,
        vatPercent: decimalToInputValue(debitNote.vatPercent ?? 0),
        isRefunded: debitNote.isRefunded === 1,
        refundAmount: decimalToInputValue(debitNote.refundAmount),
  ...
  const [lines, setLines] = useState(() => (isEdit && initialLines ? linesFromDetail(initialLines) : [emptyLine()]));
  ...
  async function startSubmit() {
    ...
    const result = isEdit
      ? await updateDebitNoteAction(companySlug, debitNote.id, payload)
      : await createDebitNoteAction(companySlug, payload);
    setPending(false);
    if (!result.ok) {
    ...
    router.push(`/${companySlug}/purchase/debit-notes/${isEdit ? debitNote.id : result.id}`);
    router.refresh();
  }
    ...
    <div className="space-y-5">
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {isEdit && (
            <div className="space-y-1.5">
              ...
              <Input className="h-11" value={debitNote.debitNoteNumber} disabled />
              <p className="text-xs text-muted-foreground">{t("debitNoteNumberHint")}</p>
            </div>
          )}
          <DualDateField id="dn-date" label={t("debitNoteDate")} value={form.debitNoteDate} onChange={(value) => update("debitNoteDate", value)} required />
          <div className="space-y-1.5">
            <Label htmlFor="dn-warehouse">{t("warehouse")}</Label>
            <CreatableSelect id="dn-warehouse" options={warehouseOptions} value={form.warehouseId} onChange={(value) => update("warehouseId", value)} placeholder={t("selectWarehouse")} />
            {fieldErrors.warehouseId?.[0] && <p className="text-xs text-destructive">{t(fieldErrors.warehouseId[0])}</p>}
          </div>

          <div className="space-y-1.5">
            <Label htmlFor="dn-party">{t("supplier")}</Label>
            <CreatableSelect id="dn-party" options={partyOptions} value={form.partyId} onChange={selectParty} placeholder={t("selectSupplier")} />
            {fieldErrors.partyId?.[0] && <p className="text-xs text-destructive">{t(fieldErrors.partyId[0])}</p>}
          </div>
          <div className="space-y-1.5">
            <Label htmlFor="dn-reference">{t("referenceNo")}</Label>
            <Input id="dn-reference" className="h-11" value={form.referenceNo} onChange={(event) => update("referenceNo", event.target.value)} placeholder={t("optional")} />
          </div>
          <div className="space-y-1.5">
            <Label htmlFor="dn-supplier-name">{t("supplierName")}</Label>
            <Input id="dn-supplier-name" className="h-11" value={form.supplierName} onChange={(event) => update("supplierName", event.target.value)} />
          </div>

          <div className="space-y-1.5 sm:col-span-2">
            <Label htmlFor="dn-supplier-address">{t("supplierAddress")}</Label>
            <Input id="dn-supplier-address" className="h-11" value={form.supplierAddress} onChange={(event) => update("supplierAddress", event.target.value)} />
          </div>
          <div className="space-y-1.5">
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        <div className="space-y-4 lg:col-span-2">
          <div className="space-y-1.5">
            <Label htmlFor="dn-notes">{t("notes")}</Label>
            <textarea
              id="dn-notes"
              rows={3}
          ...
          <div className="space-y-3 rounded-2xl border bg-background p-4">
            <div className="flex items-center gap-2">
              <Checkbox id="dn-is-refunded" checked={form.isRefunded} onCheckedChange={(checked) => update("isRefunded", checked === true)} />
              <Label htmlFor="dn-is-refunded" className="cursor-pointer">
                {t("refundedNow")}
              </Label>
            </div>
            <p className="text-xs text-muted-foreground">{t("refundedNowHint")}</p>
            {form.isRefunded && (
              <div className="grid grid-cols-2 gap-3 pt-1">
                <div className="space-y-1.5">
                  <Label htmlFor="dn-bank-account">{t("bankAccount")}</Label>
                  <CreatableSelect id="dn-bank-account" options={bankAccountOptions} value={form.bankAccountId} onChange={(value) => update("bankAccountId", value)} placeholder={t("selectBankAccount")} />
                  {fieldErrors.bankAccountId?.[0] && <p className="text-xs text-destructive">{t(fieldErrors.bankAccountId[0])}</p>}
                </div>
                <div className="space-y-1.5">
                  <Label htmlFor="dn-refund-amount">{t("refundAmount")}</Label>
                  <Input id="dn-refund-amount" type="number" min="0" step="0.01" className="h-11" value={form.refundAmount} onChange={(event) => update("refundAmount", event.target.value)} />
                </div>
              </div>
            )}
          </div>
