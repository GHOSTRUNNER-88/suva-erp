...
/**
 * Create/edit form for a Purchase Order — a dedicated page (not a Sheet)
 * because the line-items table needs real width. Shared between
 * purchase/orders/new/page.js and purchase/orders/[order]/page.js's edit
 * mode. Totals shown here are a live client-side preview using the exact
 * same lib/money.js functions the server re-runs on submit — the server
 * never trusts this component's numbers, it just recomputes independently
 * (AGENTS.md §5).
 */
export default function OrderForm({ companySlug, mode, order, initialLines, formData, prefill }) {
  ...
  const [form, setForm] = useState(() => {
    if (isEdit && order) {
      return {
        ...
        discValue: order.discType === "percent" ? decimalToInputValue(order.discPercent) : decimalToInputValue(order.discAmount),
        isVatApplicable: order.isVatApplicable === 1,
        vatPercent: decimalToInputValue(order.vatPercent ?? 0),
        notes: order.notes ?? "",
        status: order.status,
      };
    }
  ...
  const [lines, setLines] = useState(() =>
    isEdit && initialLines ? linesFromDetail(initialLines) : prefill?.lines?.length ? linesFromDetail(prefill.lines) : [emptyLine()]
  );
  ...
  async function startSubmit() {
    ...
    const result = isEdit
      ? await updatePurchaseOrderAction(companySlug, order.id, payload)
      : await createPurchaseOrderAction(companySlug, payload);
    setPending(false);
    if (!result.ok) {
    ...
    router.push(`/${companySlug}/purchase/orders/${isEdit ? order.id : result.id}`);
    router.refresh();
  }
      ...
      <div className="rounded-2xl border bg-background p-4">
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
          <div className="space-y-1.5">
            <Label htmlFor="po-order-number">{t("orderNumber")}</Label>
            <Input id="po-order-number" className="h-11" value={form.orderNumber} onChange={(event) => update("orderNumber", event.target.value)} />
            {fieldErrors.orderNumber?.[0] && <p className="text-xs text-destructive">{t(fieldErrors.orderNumber[0])}</p>}
          </div>
          <DualDateField id="po-order-date" label={t("orderDate")} value={form.orderDate} onChange={(value) => update("orderDate", value)} required />
          <DualDateField id="po-expected-date" label={t("expectedDate")} value={form.expectedDate} onChange={(value) => update("expectedDate", value)} />

          <div className="space-y-1.5">
            <Label htmlFor="po-party">{t("supplier")}</Label>
            <CreatableSelect id="po-party" options={partyOptions} value={form.partyId} onChange={selectParty} placeholder={t("selectSupplier")} />
            {fieldErrors.partyId?.[0] && <p className="text-xs text-destructive">{t(fieldErrors.partyId[0])}</p>}
          </div>
          <div className="space-y-1.5">
            <Label htmlFor="po-warehouse">{t("warehouse")}</Label>
            <CreatableSelect id="po-warehouse" options={warehouseOptions} value={form.warehouseId} onChange={(value) => update("warehouseId", value)} placeholder={t("selectWarehouse")} />
          </div>
          <div className="space-y-1.5">
            <Label htmlFor="po-pan">{t("panNumber")}</Label>
            <Input id="po-pan" className="h-11" value={form.panNumber} onChange={(event) => update("panNumber", event.target.value)} />
          </div>

          <div className="space-y-1.5 sm:col-span-2">
            <Label htmlFor="po-supplier-name">{t("supplierName")}</Label>
            <Input id="po-supplier-name" className="h-11" value={form.supplierName} onChange={(event) => update("supplierName", event.target.value)} />
          </div>
          <div className="space-y-1.5">
          ...
          <div className="space-y-1.5 sm:col-span-3">
            <Label htmlFor="po-supplier-address">{t("supplierAddress")}</Label>
            <Input id="po-supplier-address" className="h-11" value={form.supplierAddress} onChange={(event) => update("supplierAddress", event.target.value)} />
          </div>
        </div>
      </div>
